...
VIDEO_EXTENSIONS = {"avi", "m4v", "mov", "mp4", "mpeg", "webm"}
...
def save_uploaded_file(file, upload_folder):
    ...
    if extension not in MEDIA_EXTENSIONS:
        ...
        raise ValueError(f"File '{original_filename}' không đúng định dạng. Chỉ nhận: {allowed}.")

    original_root = os.path.splitext(original_filename)[0]
    safe_root = secure_filename(original_root) or "media"
    safe_name = f"{uuid4().hex}_{safe_root}.{extension}"
...
def seed_products():
    ...
    for product, discount in zip(sample_products, sample_discounts):
        product.discount_percent = discount

    sample_product_names = [product.name for product in sample_products]
    existing_sample_products = Product.query.filter(Product.name.in_(sample_product_names)).all()
    for product in existing_sample_products:
        product.seller_id = None
        product.seller_name = None

    has_sample_discounts = Product.query.filter(
        Product.name.in_(sample_product_names),
...
SCHEMA_COLUMNS = {
    "users": {
        "shop_name": {"sqlite": "VARCHAR(150)", "mssql": "NVARCHAR(150) NULL"},
        "phone": {"sqlite": "VARCHAR(30)", "mssql": "NVARCHAR(30) NULL"},
        "address": {"sqlite": "VARCHAR(255)", "mssql": "NVARCHAR(255) NULL"},
    },
    "orders": {
        "payment_method": {"sqlite": "VARCHAR(50) DEFAULT 'cash'", "mssql": "NVARCHAR(50) NULL"},
        "items": {"sqlite": "TEXT", "mssql": "NVARCHAR(MAX) NULL"},
        "customer_name": {"sqlite": "VARCHAR(150)", "mssql": "NVARCHAR(150) NULL"},
        "phone": {"sqlite": "VARCHAR(30)", "mssql": "NVARCHAR(30) NULL"},
        "address": {"sqlite": "VARCHAR(255)", "mssql": "NVARCHAR(255) NULL"},
        "seller_id": {"sqlite": "INTEGER", "mssql": "INT NULL"},
        "seller_name": {"sqlite": "VARCHAR(150)", "mssql": "NVARCHAR(150) NULL"},
    },
    "Products": {
        "category": {"sqlite": "VARCHAR(100) DEFAULT 'Sản phẩm'", "mssql": "NVARCHAR(100) NULL"},
        "discount_percent": {"sqlite": "FLOAT DEFAULT 0", "mssql": "FLOAT NULL"},
        "seller_id": {"sqlite": "INTEGER", "mssql": "INT NULL"},
        "seller_name": {"sqlite": "VARCHAR(150)", "mssql": "NVARCHAR(150) NULL"},
        "images": {"sqlite": "TEXT", "mssql": "NVARCHAR(MAX) NULL"},
        "video": {"sqlite": "VARCHAR(500)", "mssql": "NVARCHAR(500) NULL"},
        "stock": {"sqlite": "INTEGER DEFAULT 0", "mssql": "INT NULL"},
    },
}


def ensure_sql_server_database():
    if os.getenv("DATABASE_URL") or not Config.SQLALCHEMY_DATABASE_URI.startswith("mssql+pyodbc"):
        return
    ...
    try:
        ...
        raise RuntimeError(
            "Chưa cài pyodbc. Chạy: pip install -r backend/requirements-sqlserver.txt"
        ) from exc

    if Config.SQLSERVER_USERNAME and Config.SQLSERVER_PASSWORD:
        auth = f"UID={Config.SQLSERVER_USERNAME};PWD={Config.SQLSERVER_PASSWORD}"
    else:
    ...
    connection = (
        ...
        "DATABASE=master;"
        f"{auth};"
        f"TrustServerCertificate={Config.SQLSERVER_TRUST_CERTIFICATE};"
    )

    database_name = Config.SQLSERVER_DATABASE
    safe_database_name = database_name.replace("]", "]]")

    try:
        with pyodbc.connect(connection, autocommit=True, timeout=5) as connection_handle:
        ...
        raise RuntimeError(
            f"Không kết nối được SQL Server {Config.SQLSERVER_SERVER}. "
            "Hãy kiểm tra SQL Server Express đang chạy và đã cài ODBC Driver."
        ) from exc
...
def get_table_columns(table_name):
    ...
    actual_table = next(
        (name for name in inspector.get_table_names() if name.lower() == table_name.lower()),
        None,
    )
    ...
    return actual_table, {column["name"].lower() for column in columns}


def get_column_definition(definitions):
    dialect = db.engine.dialect.name
    return definitions.get(dialect) or definitions.get("default")


def ensure_database_schema():
    for table_name, columns in SCHEMA_COLUMNS.items():
        actual_table, existing_columns = get_table_columns(table_name)
        ...
        for column_name, definitions in columns.items():
            ...
            column_definition = get_column_definition(definitions)

            if not column_definition:
                continue

            statement = (
                f"ALTER TABLE {quote_identifier(actual_table)} "
                f"ADD {quote_identifier(column_name)} {column_definition}"
            )
...
def create_app():
    ...
    def handle_large_upload(error):
        return jsonify({"message": f"File vượt quá {MAX_UPLOAD_SIZE_MB}MB."}), 413

    @app.route("/api/upload", methods=["POST"])
    def upload():
        files = [
            file
            for field_name in ("files", "file", "media")
            for file in request.files.getlist(field_name)
            if file and file.filename
        ]

        if not files:
            return jsonify({"message": "Chưa chọn file để tải lên."}), 400

        try:
            saved_files = [save_uploaded_file(file, app.config["UPLOAD_FOLDER"]) for file in files]
        except ValueError as exc:
            return jsonify({"message": str(exc)}), 400
        except Exception:
            return jsonify({"message": "Không lưu được file tải lên. Kiểm tra thư mục backend/uploads."}), 500

        images = [item["url"] for item in saved_files if item["type"] == "image"]
        videos = [item["url"] for item in saved_files if item["type"] == "video"]
